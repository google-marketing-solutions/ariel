...
export function renderTimeline(videoData, videoDuration, speakers) {
  ...
  const originalSpeakerLabelsContainer = document.getElementById(
    'original-speaker-labels',
  );
  const originalUtterancesTracksContainer = document.getElementById(
    'original-utterances-tracks',
  );
  const translatedSpeakerLabelsContainer = document.getElementById(
    'translated-speaker-labels',
  );
  const translatedUtterancesTracksContainer = document.getElementById(
    'translated-utterances-tracks',
  );
  ...
  const timelineWidth =
    timelineContainer.offsetWidth - SPEAKER_LABEL_COLUMN_WIDTH; // Subtract label column width
  ...
  videoData.utterances.forEach((utterance, index) => {
    const originalTrack = document.getElementById(
      `original-speaker-track-${utterance.speaker.voice.replace(/[^a-zA-Z0-9-_]/g, '')}`,
    ); // Use sanitized voice for ID
    if (originalTrack) {
  ...
  videoData.utterances.forEach((utterance, index) => {
    const translatedTrack = document.getElementById(
      `translated-speaker-track-${utterance.speaker.voice.replace(/[^a-zA-Z0-9-_]/g, '')}`,
    ); // Use sanitized voice for ID
    if (translatedTrack) {
      ...
      if (utterance.removed) {
        ...
        const initialOverlapMessages = checkOverlap(
          utterance,
          videoData.utterances,
        );
      ...
      translatedBlock.addEventListener('mousedown', e => {
        ...
        function handleMouseMove(e) {
          ...
          if (utteranceEditor.style.display === 'block') {
            const editedUtteranceId = utteranceEditor.dataset.utteranceId;
            const editedUtterance = videoData.utterances.find(
              u => u.id === editedUtteranceId,
            );
            ...
            if (editedUtterance) {
              ...
              if (editedUtteranceId === utterance.id) {
                document.getElementById('translated-start-time-input').value =
                  newStartTime.toFixed(2);
                document.getElementById('translated-end-time-input').value =
                  newEndTime.toFixed(2);
              }

              const overlapMessages = checkOverlap(
                editedUtterance,
                videoData.utterances,
              );
