...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const selectVideoComputerBtn = document.getElementById(
    'select-video-computer',
  );
  ...
  const editVideoSettingsBtn = document.getElementById(
    'edit-video-settings-btn',
  );
  ...
  const generatedVideoPreview = document.getElementById(
    'generated-video-preview',
  );
  const downloadVideoButton = document.getElementById('download-video-button');
  const downloadVocalsButton = document.getElementById(
    'download-vocals-button',
  );
  const downloadVocalsMusicButton = document.getElementById(
    'download-vocals-music-button',
  );
  const goBackToEditingButton = document.getElementById(
    'go-back-to-editing-button',
  );
  const startOverButton = document.getElementById('start-over-button');

  // Completed Video Modal
  const completedVideoModal = new bootstrap.Modal(
    document.getElementById('completed-video-modal'),
  );
  ...
  const thinkingPopupContent = document.querySelector(
  ...
  function startThinkingAnimation() {
    ...
    phraseChangeInterval = setInterval(() => {
      const currentPhrase = thinkingWord.textContent;
  ...
  function validateStartProcessing() {
    ...
    startProcessingBtn.disabled = !(
      videoSelected &&
      speakersSelected &&
      originalLangSelected &&
      translationLangSelected
    );
  }
  ...
  fetchLanguages(originalLanguage, translationLanguage).catch(error => {
    ...
    showToast(
      'Could not load language data. Please refresh the page.',
      'error',
    );
  });
  ...
  speakerModalEl.addEventListener('hidden.bs.modal', () => {
    ...
    currentEditingVoiceName = null;
    speakerModalEl.querySelector('.modal-title').textContent =
  ...
  editVoiceSearch.addEventListener('input', () =>
    renderVoiceList(
      editVoiceListModal,
      editVoiceSearch,
      'edit-gender-filter',
  ...
  document
    .querySelectorAll('input[name="edit-gender-filter"]')
    .forEach(radio => {
      radio.addEventListener('change', () =>
        renderVoiceList(
          editVoiceListModal,
  ...
  speakerList.addEventListener('click', e => {
    ...
    if (editBtn) {
      ...
      speakerModalEl.querySelector('.modal-title').textContent =
        'Edit Speaker Voice';
      speakerModalEl.querySelector('#speaker-name-input').value =
        speakerToEdit.name;
    ...
    if (e.target.classList.contains('btn-close')) {
      ...
      if (speakerCard) {
        ...
        speakers.forEach((speaker, index) => {
          ...
          if (
            speaker.name.startsWith('Speaker ') &&
            !isNaN(parseInt(speaker.name.split(' ')[1]))
          ) {
  ...
  startProcessingBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const closeFloatingVideoBtn = document.getElementById(
      ...
      const originalLanguageName =
        originalLanguage.options[originalLanguage.selectedIndex].text;
      const translationLanguageName =
        translationLanguage.options[translationLanguage.selectedIndex].text;
  ...
  function makeDraggable(element) {
    ...
    element.addEventListener('mousedown', e => {
      if (
        e.target === element ||
        e.target.classList.contains('results-video')
      ) {
        isDragging = true;
  ...
  function updateResetButtonVisibility() {
    ...
    const hasChanges = currentVideoData.utterances.some(
      u =>
  ...
  function renderVideoSettingsEditor() {
    ...
    fetch('static/languages.json')
      ...
      .then(languages => {
        ...
        const previewLanguages = languages.filter(
        ...
        document
          .getElementById('cancel-edit-settings')
        ...
        document.querySelectorAll('.edit-speaker-voice-btn').forEach(button => {
          button.addEventListener('click', e => {
            ...
            const speakerModalEl = document.getElementById('speaker-modal');
            speakerModalEl.querySelector('.modal-title').textContent =
              'Change Speaker Voice'; // Change modal title
            speakerModalEl.querySelector('#speaker-name-input').value =
              speakerToEdit.name; // Populate name input
        ...
        document
          .getElementById('submit-edit-settings')
          .addEventListener('click', async () => {
            const newOriginalLanguage = document.getElementById(
              'edit-original-language',
            ).value;
            const newTranslateLanguage = document.getElementById(
              'edit-target-language',
            ).value;
            const originalLangChanged =
              newOriginalLanguage !== currentVideoData.original_language;
            const translateLangChanged =
            ...
            try {
              if (originalLangChanged) {
                ...
                const changedSpeakers = speakers.filter(
                  s =>
                    !currentVideoData.speakers.find(
                      cs => cs.speaker_id === s.id && cs.voice === s.voice,
                    ),
                );
                ...
                const dubbingPromises = currentVideoData.utterances
                  ...
                  .map((utterance, _) => {
                    const originalIndex = currentVideoData.utterances.findIndex(
                      u_orig => u_orig.id === utterance.id,
                    );
                    return runRegenerateDubbing(
                      currentVideoData,
                      utterance,
                      originalIndex,
                      utterance.instructions,
              ...
              const translationLanguageName = document.getElementById(
                'edit-target-language',
              ).selectedOptions[0].text;
              videoSettingsContent.innerHTML = `
                            <p><strong>Original Language:</strong> ${originalLanguageName}</p>
