...
"""Dubble proof of concept for video dubbing."""
...
def create_default_utterances(config: DubbleConfig) -> list[dict[str, Any]]:
  ...
  config.script = {
      ...
      "tone": "Upbeat",
      "vocal_style_prompt": (
          "A friendly and energetic young adult male voice, speaking with a"
          " fast-paced, upbeat delivery. The tone is enthusiastic and"
          " persuasive, perfect for an advertisement targeting a youthful"
          " audience, making the service sound exciting and convenient."
      ),
      "suggested_voice": "",
  }
...
def generate_single_utterance_speech(
    utterance_data: dict, config: DubbleConfig, job_dir: str
):
  ...
  generation_id = f"{utterance.speaker_id}_{utterance.start:.2f}".replace(
      ".", "_"
  )

  audio_clip, utterance = llm.generate_timed_speech_for_utterance(
      generation_id=generation_id, utterance=utterance, config=config
  )
...
def assemble_final_video(utterances: list, config: DubbleConfig):
  ...
  for utterance in utterances:
    ...
    if not audio_url or start_time is None:
      logger.warning(
          "Skipping utterance due to missing audio_url or start time:"
          f" {utterance}"
      )
      continue
    ...
    if os.path.exists(full_audio_path):
      ...
      logger.warning(
          f"Audio file not found for utterance: {utterance}. Path checked:"
          f" {full_audio_path}"
      )
  logger.debug(f"Times vocal clips {timed_vocal_clips}")
  final_video_path = assemble_video(
      config=config, timed_vocal_clips=timed_vocal_clips
  )
