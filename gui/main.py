...
"""Dubble: A FastAPI application for an AI Dubbing Service."""
...
async def read_root(request: Request):
  ...
  return templates.TemplateResponse(
      "index.html",
      {"request": request, "default_prompts": config.prompt_library},
  )
...
async def clean_outputs():
  ...
  try:
    ...
    return JSONResponse(
        {"status": "success", "message": "Outputs & Uploas directory cleaned."}
    )
  except Exception as e:
    logger.error(f"Error cleaning outputs directory: {e}")
    raise HTTPException(
        status_code=500, detail=f"Error cleaning outputs directory: {e}"
    )
...
async def start_process(
  ...
  if video_file and video_file.filename:
    ...
    file_path = dubble_logic.download_from_gcs(
        gcs_url=gcs_path, file_path=file_path
    )

  if not file_path:
    raise HTTPException(
        status_code=400, detail="No video file or GCS path provided."
    )
...
async def generate_speech(
  ...
  try:
    new_translated_text = dubble_logic.translate_utterance(
        utterance, state["config"]
    )
...
async def generate_speech(
    job_id: str = Form(...), index: int = Form(...), utterance: str = Form(...)
):
  ...
  try:
    audio_path, new_translated_text = (
        dubble_logic.generate_single_utterance_speech(
            utterance, state["config"], job_dir
        )
    )
    ...
    return JSONResponse({
        "audio_url": f"/outputs/{job_id}/{os.path.basename(audio_path)}",
        "translated_text": new_translated_text,
        "original_text": utterance["original_text"],
    })
...
async def approve_speech(
  ...
  if not os.path.exists(full_audio_path):
    raise HTTPException(
        status_code=404, detail="Approved audio file not found."
    )
...
async def add_utterance(
    job_id: str = Form(...), index=Form(...), utterance: str = Form(...)
):
...
async def remove_utterance(
    job_id: str = Form(...), utterance_id: str = Form(...)
):
...
def download(filename: str):
  ...
  return FileResponse(
      path=path, filename=filename, headers=headers
  )  # provide filename, media_type is optional
...
async def apply_to_single_video(
  ...
  try:
    ...
    raise HTTPException(
        status_code=500,
        detail=f"Failed to process video {video_url}. Reason: {e}",
    )
